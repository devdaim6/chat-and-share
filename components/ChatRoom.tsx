...
const ChatRoom: React.FC<ChatRoomProps> = ({ roomId, username }) => {
  ...
  const handleScroll = useCallback(() => {
    ...
    // Check if we're within 25% of the top to prefetch more messages
    const scrollThreshold = container.scrollHeight * 0.25;
    if (container.scrollTop < scrollThreshold && hasMore && !loadingMore) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-0 md:p-0 flex flex-col">
      {/* Fixed Header */}
      <div className="fixed md:top-10 w-screen z-10 pt-0 mb-0">
        <Card className="shadow-lg bg-white/95 backdrop-blur-sm">
          <CardHeader className="flex flex-row items-center justify-between border-b pb-4">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">ID: {roomId}</p>
            </div>
      ...
      <div className="flex-1 flex flex-col" style={{ height: "calc(100vh - 140px)" }}>
        <Card className="flex-1 flex flex-col  transition-all duration-300    mt-[4.4rem]">
          <CardContent
          ...
          >
            ...
            {userScrolled && (
              <Button
                variant="outline"
                size="icon"
                className="fixed bottom-28 right-8 z-[999] rounded-full shadow-lg bg-white hover:bg-blue-50"
      ...
      <div className="sticky md:bottom-0 md:pt-12 bottom-12 bg-white border-t">
        <form
          onSubmit={handleSendMessage}
          className="p-4 bg-gray-50/50 backdrop-blur-sm"
        >
        ...
        {showEmojiPicker && (
          <div className="emoji-picker absolute bottom-full right-4 z-50">
